...
// 系统提示。**五件事必须说清楚，缺一个模型就会答歪**（和分诊层的 prompt 同一个道理）：
//   ① 它的任务是"用工具查证"，不是"凭印象回答"
//   ② 资料不够时**必须说做不到**，而且要说出缺什么 —— 这是赛题"可验收性"的落点
//   ③ 结论要**带出处**，而且格式固定成 `#会话号·段号`
//   ④ 不要把工具结果原文照抄给用户，要归纳
//   ⑤ 工具调用要节制
//
// 【③ 为什么必须写死格式】"带出处"这三个字模型会理解成很多种写法
// （「第 3 段」「会话 9002」「14:01 那会儿」），而 `--verify-report` 只认
// `#会话·段`。两边不一致的后果是**最坏的那种**：模型真的标了出处，
// 核对器却报"一处出处都没有" —— 于是人以为模型没标，去改 prompt，
// 而真正的问题是格式没对齐。
// 所以格式在 prompt 里写死，并且明确告诉它"这个格式是机器核对的" ——
// 说了这句之后它写段号会谨慎得多（编一个会被抓，不如不标）。
const char* kSystemPrompt =
    ...
    u8"3. **结论要带出处，而且格式必须固定：`#会话号·段号`。**\n"
    u8"   例：`接口文档需要重写（#9002·2）`。段号就是 get_session 结果里每段的 seq。\n"
    u8"   ⚠️ 这个格式是**机器核对的**：报告出来之后程序会把每处 `#会话·段` 回库里查，\n"
    u8"      对不上的会当场标成\"编造或写错\"。所以**只为你真的看到过的段落标出处** ——\n"
    u8"      随手写一个段号比不写更糟：它会被查出来，而且看起来很像真的。\n"
    u8"   ⚠️ 查不到具体段落的结论（比如\"这周一共开了三场会\"）就不标，不要硬凑。\n"
