...
>
> 最后更新：2026-09-19（**第 5 阶段 Agent 化 5.1–5.4 完成 + 2.12 判断缓存 + 2.11 类型化提问 +
> 一个真 bug「同一实体被问两遍」修掉**；下一步 = **5.5 Action 落库 / 5.7 Evidence / 5.8 演示脚本**，
> 或者按用户"先做一半"的意思继续剩下的）
...
**当前阶段**：第 5 阶段 Agent 化 **5.1–5.4、5.6 已完成**。
**下一步 = 5.5 Action 落库（跨会话聚合 Todo/Doing/Done）**，之后 5.7 Evidence、5.8 演示脚本。
...
> ⚠️ **用户明确要求 GUI（第 4 阶段）最后做**，先把 agent 功能做全。
...
| **第 5 阶段** Agent 化（**插在第 4 阶段之前**） | 5.1 Tool Registry ✅ / 5.2 规划循环 ✅ / 5.3 `--search` 检索 ✅ / 5.4 写工具+闸 ✅ / **5.5 Action 落库 ← 下一步** / 5.6 `--report` 派活 ✅ / 5.7 Evidence / 5.8 Demo | **5.1–5.4、5.6 ✅**；5.5 / 5.7 / 5.8 未做 |

### 刚做完（2026-09-19，Agent 阶段 + 判断缓存）

| 提交 | 内容 | 一句话 |
|---|---|---|
| `459b268` | **2.11 按类型问对的问题** | 人名问身份 / 项目问用途 / 产品问定位，不再一律「这是重要概念吗」 |
| `a4cd28c` | 修 R7：报告动词后面的中文项目名整条丢失 | `他说凤凰项目的…` 抽不出东西（左扫越过了「说」） |
...
**验证现状**：自检 41 组全过（`--selftest`，秒级、不联网、不加载模型）；
`agent_tools_check` / `zh_extract_cases` / `verify_memory` / `loop_evidence` 全过。
...
| 中文抽取在真实 ASR 输出上的准确率 | ⚠️ 只验了手写语料 |
| 云端翻译 / 云端摘要 | ⚠️ Key 打过一次真跑，未做系统回归 |
| 麦克风双路 | ⚠️ 未验 |
...
| 09-19 | **诊断工具撒谎（第四次）：`loop_evidence.py` 自己拼 `initial_prompt`** | 它用 SQL 自己拼"下一场会有哪些词"，**漏了去重**，打出 `Marco、EnglishPod、Erika、Erika、VAD`；真实路径 `--terms` 只给一个 `Erika`。讽刺的是这个文件的**开头就写着**"抽取那一步刻意调同一个 exe，重写一份就是两个实现，两边迟早走散" —— 它违反了它自己写的原则。撒谎方向最坏：**让好库看起来像坏库**。已改成直接调 `--terms`，并加了**交叉核对**（重建 vs 真实，不一致就报警） |
