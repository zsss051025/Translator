...
>
> 最后更新：2026-09-17（第 4 阶段方案已定 + 三个决策；下一个动作 = 2.5b 收尾，然后 4.1/4.2）
...
**当前阶段**：第 2 阶段（知识库骨架）2.1~2.6 已完成、闭环已闭上；
**第 4 阶段（桌面产品化）方案已定但要等 2.5b 收尾**。
`PROJECT.md` §7 第 4 阶段里有：三个决策、可行性证据、11 步顺序、
架构约束（6 条）、以及**产品形态举例（例 0~6）**——那份是需求，动 GUI 之前先读它。
...
| | （2.5b 交付物 UTF-8 净化 —— 半成品，见下） | ⏸ **暂停** |

### 下一个动作（具体到文件）

**2.5b 交付物 UTF-8 净化**（小，先把半成品收尾）

- 已完成一半：`inc/Utf8.h` / `src/Utf8.cpp` **已写好但未接进构建**（未跟踪状态，对现有行为零影响）
- 剩下：加进 `CMakeLists.txt`；在 `DeliverableWriter::write_file()`（四个交付物的**唯一出口**）里
  调 `utf8::sanitize()` 并**大声报一行**；`LlmSummarizer::summarize()` 解析前也过一遍；补自检用例
- 触发条件：混元做摘要时吐半个字符的 token（`meeting-42.md` 的 `询问你\xe7`）。
  **云端 DeepSeek 一般不会触发**（会话 #44 的文件是干净的）

之后是 **4.1 + 4.2**（多助手的地基）：
- 4.1 默认输出改 `%USERPROFILE%\Documents\AudioTranslator\` **+ 按助手分目录**
  —— 后者是硬前提，见下面 §四 里的"静默覆盖"那条
- 4.2 第一个持久化配置 `config.json` + 助手注册表 + 记住上次用的

> **完整方案（含三个决策、11 步顺序、产品形态举例）在 `PROJECT.md` §7 第 4 阶段。**
> 这里不重复，只记状态。
...
**工作树状态**：干净，除了两个**未跟踪**文件 —— `inc/Utf8.h` / `src/Utf8.cpp`
（2.5b 的半成品，**没接进 CMakeLists、没被任何代码调用**，对现有行为零影响）。
下次接着做 2.5b 时从它们开始。
...
| 时间 | 事 | 关键点 |
|---|---|---|
| 09-17 | **三个产品决策定了（第 4 阶段）** | ①**助手 = 一整套独立**（知识/会话/纪要/任务/语言/是否上云跟着走；**模型路径和 key 是全局的**）②GUI 走 **Win32 + WebView2** ③**模型打包进安装包**。完整理由与证据在 `PROJECT.md` §7 第 4 阶段 |
| 09-17 | **GUI 可行性是查出来的，不是猜的** | WebView2 运行时**本机已装**（153.0.4234.32 + 注册表项）；vcpkg 有 `webview2` port 而项目已在用 vcpkg；**STA+消息泵模式项目已有**（`SubtitleWindow` 自己开线程 + `GetMessageW`）；纪要查看器**白送**（交付物本来就是自包含单文件 HTML） |
| 09-17 | **多助手有个静默覆盖的坑，必须先堵** | `DeliverableWriter.cpp:960` 用 `session-<id>` 命名目录，而 id 是**每个库各自编号**的 → 两个助手各有会话 #5 就写进同一目录，**后写的静默盖掉先写的**。所以 4.1 必须包含"输出目录按助手分" |
| 09-17 | **GUI 下"结束提问"会直接跳过，必须自己弹窗** | 现有判断 `if (!stdin_is_tty()) ask = false;`，GUI 里 stdin 不是终端 → 静默跳过。**接缝 2.4 就留好了**（`run_confirmation` 收 `LineReader` 回调），不用重构 |
| 09-17 | **GUI 阶段不要改 `/SUBSYSTEM:WINDOWS`** | 验证阶梯（`--selftest`/`--wav`/`--gaps`/`--extract`/`verify_memory.py`）全走控制台，改了会全废。做法：隐藏控制台窗口 + 日志转文件/UI 面板 |
| 09-17 | **我造成的数据丢失事故（已修已恢复）** | `tools/ab_translation_constraint.py` 第一版直接 `DELETE FROM knowledge`，我跑了好几遍验证"按段过滤"，**把用户 4 条 confirmed 全删了**。已改成一次性 scratch 库 + 拒绝在有数据的库上运行；已从会话 #43 重新抽取确认恢复 |
| 09-17 | **术语约束会凭空编造专名（回归，已修）** | 用户真跑会话 #44 发现：`and wife get ready to go` → 「埃丽卡和马可准备出发了」，原文一个名字都没有。**对照实验证实**是约束造成（无约束时忠实、有约束时编造）。根因：约束对**每一段**都生效，即使那段没有该词。修法：`ITranslator::glossary_for_text()` 只约束本段真出现过的术语 |
| 09-17 | **零问题时静默 = 和坏了长得一样** | 会话 #44 日志里一行 `[确认]` 都没有 —— 因为抽出的 3 个名字上一场都确认过了。行为对，但排查时只能靠读代码。改成零问题也打一行 |
| 09-16 | 2.6 **闭环缺了一整个环节，是用户真跑才暴露的** | 2.1~2.5 全建立在"库里已经有知识"上，而**没有任何代码把会话写进去**。真跑一场 16 段、DeepSeek 摘要成功的会话后 `knowledge` 表仍是 0 行。**`PROJECT.md §6.4①` 写了设计，但 §7 步骤表里没有对应的一步 —— 没进步骤表就等于不存在** |
| 09-16 | 2.6 **只修数据流一环 ≠ 修好** | 真数据报"抽取器说 2 次、库里 1 次"。改了 `upsert` 采信 `hits`，**重跑还是 1** —— 真相是 `save_candidates` 压根没传 `c.hits` |
| 09-16 | 2.6 **句首大写不是专名证据** | 英文句首必然大写那是语法。实测 jfk.wav 会话抽出 **0 个** —— `Ask`/`What`/`And` 一个都没误抽（反向对照） |
| 09-16 | 2.5 **差点用不现实的探针得出错误结论** | 把术语值设成 `马可波罗` 看到没照做，就下结论"约束没权威"。换成真实失败句立刻翻过来 |
| 09-16 | 2.4 **"提问稀缺"要有个地方记"问过了"** | `asked_count` + `kMaxAsks = 2`，**跳过也计数**。实测：第 1 场问 3 个 → 第 2 场 1 个 → 第 3 场 0 个 |
