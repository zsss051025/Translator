...
| 时间 | 事 | 关键点 |
|---|---|---|
| 09-17 | 2.5b **我诊断错了，是靠"数字对不上账"纠正的** | 我一直说 `meeting-42.md` 坏掉是"混元吐了半个字符的 token"。加完净化重新导出时发现：**净化器报了 6 处坏字节，但上游"模型回复里有 N 处非法 UTF-8"一行都没打** —— 模型回复是干净的。真病根是 `LlmSummarizer` 里的 `item.find_first_of(u8"：:")`：**它按单字节比较**，而 `u8"："` 是 `EF BC 9A`，于是"0x9A"也算匹配 —— 而 **「的」= `E7 9A 84`，第二个字节就是 0x9A** → 一刀切在字符中间。实测 `询问你` + `\xe7` 与当年产物**逐字节一致** |
| 09-17 | 2.5b **`0x9A`/`0xBC` 是极常见的汉字续字节** | 的/地/得/等… 所以这个 bug 在实际输出里**几乎必然命中**。顺带 `substr(colon + 1)` 也错了 —— 全角冒号占 3 字节，`+1` 从下一个字中间开始 |
| 09-17 | 2.5b **往后的教训：报警数量与上游不匹配，就是有第二个入口** | 如果只有一个产生点，"净化器报警数" 应当等于 "上游报警数"。不相等说明**还有别的地方在造脏数据**，而那个地方才是真凶。这条值得当通用排查法用 |
| 09-17 | 2.5b **新用例确认过"修复前会失败"** | 临时把 `find_first_of` 退回去，自检报 `标题被切坏了：'询问你�'` —— 正是当年那个症状。**这才是回归线，不是安慰用例** |
| 09-17 | 2.5b **净化闸留着，但理由改了** | 具体 bug 修了，但"按字节比多字节分隔符""按字节截断"这类入口不止一个，而且**自检全绿、打开文件才发现**。所以留一道最后防线：保证写出去的文件永远合法。取向是**净化 + 大声报告**（拒绝写盘会让用户一份交付物都拿不到） |
...
| 09-17 | **GUI 下"结束提问"会直接跳过，必须自己弹窗** | 现有判断 `if (!stdin_is_tty()) ask = false;`。**接缝 2.4 就留好了**（`run_confirmation` 收 `LineReader` 回调），不用重构 |
| 09-17 | **我造成的数据丢失事故（已修已恢复）** | `tools/ab_translation_constraint.py` 第一版直接 `DELETE FROM knowledge`，我跑了好几遍验证"按段过滤"，**把用户 4 条 confirmed 全删了**。已改成一次性 scratch 库 + 拒绝在有数据的库上运行 |
...
| 09-16 | 2.6 **闭环缺了一整个环节，是用户真跑才暴露的** | 2.1~2.5 全建立在"库里已经有知识"上，而**没有任何代码把会话写进去**。**设计文档里写了、执行步骤表里没有 = 不存在** |
| 09-16 | 2.6 **只修数据流一环 ≠ 修好** | 真数据报"抽取器说 2 次、库里 1 次"。改了 `upsert` 采信 `hits`，**重跑还是 1** —— 真相是 `save_candidates` 压根没传 `c.hits` |
| 09-16 | 2.6 **句首大写不是专名证据** | jfk.wav 会话抽出 **0 个** —— `Ask`/`What`/`And` 一个都没误抽（反向对照） |
| 09-16 | 2.5 **真实失败案例在另一个库里** | "会话 #11 的 Erica/埃里卡"在 `translations.db`，不在 `t.db`。仓库里 **8 个 .db 文件**而 `--db` 是相对路径 |
