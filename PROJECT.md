...
| # | 做什么 | 验证级别 | 状态 |
|---|---|---|---|
| 0.1 | **假行动项校验器**（P0 #8）：**不只改 prompt，加一个纯函数校验器** —— 过场语/寒暄开头、无可执行动作词的条目一律丢弃；`due` 无文本依据则清空 | **L1** | ✅ **已完成**（自检 7/7 + 1） |
| 0.2 | **术语约束翻译输出**（P0 #7）：让术语表同时进入译文 prompt，不再只管识别。**先用现有 `terms.sample.txt`，不依赖知识库** | L1 + L3 | 待做（依赖 1.1 的 A/B 能力） |
...
| # | 做什么 | 验证级别 | 完成定义 |
|---|---|---|---|
| 1.1 | **`--wav <file>` 文件输入**（P1 #8）：摘 `da97d96` 的 `inc/wav_reader.h`（WAV 读入 + 重采样到 16k），做成正经参数——不要那笔提交里的菜单选项和写死的 `sleep(5)` | L2 | `--wav x.wav` 能跑完 识别→翻译→交付物 |
| 1.2 | **去掉启动时的后端选择题**（P1 #11）：改由 `--summarizer` / `--api-key` / 默认值自动决定 | L2 | 无参数启动后**不需要任何输入** |
