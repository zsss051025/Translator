...
| # | 做什么 | 要点 |
|---|---|---|
| 4.1 | **默认输出路径改到用户目录** | 现在是**相对当前目录**的 `deliverables/`。双击启动时工作目录不确定 → 会写到奇怪的地方，甚至在 `Program Files` 下因无权限直接失败。改成 `%USERPROFILE%\Documents\AudioTranslator\`，并可在设置里改 |
| 4.2 | **API key 安全存储** | 不能明文写 ini。用 Windows 凭据管理器（DPAPI）。且**首次运行不强制填** —— 不填就走离线路径（§1.3 红线） |
| 4.3 | **GUI 壳**：托盘图标 + 设置界面 | 托盘右键：开始/停止、打开纪要目录、设置、退出。用 `WinMain`（`/SUBSYSTEM:WINDOWS`）去掉黑窗口。首次运行引导只问两件事：输出目录、要不要填 key |
...
| 15 | **默认输出路径改到用户目录** | 见 §7 第 4 阶段 4.1。现在是相对当前目录，**双击启动会写到奇怪的地方** |
| 16 | **API key 安全存储 + 桌面 GUI 壳** | 见 §7 第 4 阶段 4.2 / 4.3。**key 首次运行不强制填**（§1.3 红线） |
