...
| **`PRODUCT_LOOP.md`** | **闭环审查 + 实现清单**：产品闭环通不通、有哪八个结构性漏洞、已完成/未完成分别怎么实现的。**要评估"这东西成不成"、要写方案或答辩时读** |
...
| **评估"这东西成不成" / 写方案、答辩** | **`PRODUCT_LOOP.md`**（闭环审查 + 八个结构性漏洞 + 完成/未完成清单） |
...
| **`python tools\zh_extract_cases.py`** | **中文专名抽取回归**（手写中文会议片段，一次性 scratch 库） | 该抽到的 8 个没漏 / 不该抽到的 9 个没误报 |
| **`--tools`** / **`--tools <名字> [--args ...]`** | **Agent 工具层**：看 schema，或手动跑一个工具 | 打印 5 个工具的定义；跑一次给出 JSON 结果 + 一行摘要 |
| **`--terms`** | **长期记忆到底有没有进提示词**（§6.5 红线的可观测面）。不加载任何模型，秒级 | 打印 ①真实的 `initial_prompt` 那串 ②逐条翻译约束 ③摘要背景 |
...
> **这一节是产品故事的主干。** 竞争评审要看的"闭环学习"就是 §6.4①（自动发现）
> → §6.6（发现缺口）→ §6.7（主动询问）→ §6.4③（含义入库）→ 三条腿复用（§6.4②）。
> 缺任何一环，"第一次不知道 → 用户教它 → 第二次知道 → 第三次还能利用历史"这个故事就断。
> 三环的完成度见 `PRODUCT_LOOP.md`。
...
**R1 的三条收紧判据（2026-09-17 补，2.6c）**

R1 原本只要求"该词在非句首位置大写出现过一次"。实测在一场 53 段的真实英文播客上，
这条松到不能接受 —— 抽出 8 个假专名（`down` `Keep` `Midnight` `movies` `Speaking`
`Learners` `Exactly` `preview`），而且**被用户逐条确认后真的进了 Whisper 的
`initial_prompt`**，也就是下一次识别会被主动往这些普通词上带。
现在一个键要通过 R1，必须额外满足下面任一条：

| 判据 | 内容 | 为什么 |
|---|---|---|
| **① 强形状** | 词中大写 / 连续 ≥2 个大写 / 在引号里（即 R2~R4） | 这几种形状本身就几乎只属于专名，不需要再看次数 |
| **② 佐证** | 在"报道动词"或人名句式里出现过一次（`said X` / `asked X` / `told X` / `my name is X` …） | 一句话就能定性，比数次数可靠得多 |
| **③ 次数 + 从不小写** | 非句首大写出现 **≥2 次**，且**从未**以小写形式出现过 | 普通词在自然语流里必然会有小写出现，"只在大写时出现"才是专名的特征 |

**R3（连续大写 = 缩写）的两条收紧（2026-09-17 补，用户真跑会话 #13）**

会话 #13 的段 98 是播客的小标题 `PUTTING IT TOGETHER`。整行大写里，
8 个字母的 `TOGETHER` 满足"连续 ≥2 个大写"，于是被判成缩写抽了出来，
还被问了一句「第一次听到「TOGETHER」。这个词的写法对吗？」。

| 收紧 | 内容 | 理由 |
|---|---|---|
| **限长** | 全大写当缩写**只在 ≤5 个字母时成立** | 真缩写没有超过 5 个的（CEO/CFO/KPI/OKR/SLA/SDK/HR/AI 全在 4 以内），而"被大写强调的普通词"可以有任意长度 —— **长度就是这两者的分界** |
| **整段标题作废** | 一段里**每个词都全大写**且 ≥2 个词 → 这段在"大写"上零信息量，整段不提供任何证据 | 整行大写是**排版**，不是专名的形状：同一个词在正文里就是普通小写词。与其逐个词猜哪个像专名，不如整段作废 |

> 单个词的段（`OKAY`）不整段作废 —— 可能真的是在喊一个名字，也可能就是个缩写，
> 信息不足，不据此下结论，交给别的判据处理。

**三条判据各自都是必需的 —— 每一条都被会话 #47 里一个真实词证伪过一次**
（下面的"段首/非句首"是 `python tools\extract_case_probe.py --db <库> --session 47` 打出来的，
`session_id=47` 的原文可以直接查回来）：

| 词 | 它在 #47 里的大写形态 | 拦住它的是哪条 | 留下它的是哪条 |
|---|---|---|---|
| `TV` | 连续大写 | | **①** 强形状 |
| `EnglishPod` | 词中大写 `P` | | **①** 强形状 |
| `Marco` | 非句首 **1** 次（`My name is Marco.`） | | **②** 佐证（`my name is`）。ⓘ 它非句首大写只有 1 次，**只靠 ③ 会被误杀** |
| `Erica` | 段首 1 次 + 非句首 **2** 次，**从未小写** | | **③** 两条都满足 |
| `keep` | `Keep` 段首 3 次、非句首**只有 1 次** | **③ 的次数那一半** | |
| `down` | `Down` 非句首 **2 次**（`Keep It Down.`）—— **但同一场里 `keep it down` 全小写出现 4 次** | **③ 的"从不小写"那一半** | |
| `midnight` / `speaking` / `learners` / `downs` / `movies` | 非句首**各只有 1 次**（`It's After Midnight.` / `Yeah, Speaking of Movies,` / `Hello English Learners…` / `…use Keep It Downs.` / `Movies`(7)） | **③ 的次数那一半** | |
| `exactly` | `Exactly` **是段首**（`at the movies. Exactly.` 里段首那次），非句首 0 次 | **③ 的次数那一半** | |
| `preview` | 只有全小写形式 | 连非句首大写都没有 | |

> 这张表的意义：**判据 ③ 的两半各自都是必需的，而且各有一个真实词证明**——
> `keep` 证明"次数 ≥2"不能少（非句首 1 次就够的话它会进来），
> `down` 证明"从不小写"不能少（非句首 2 次但小写 4 次）。
> 我实测过：把"从不小写"那半去掉，自检立刻报 `不该抽到却抽了「down」`。
> 判据 ②（佐证）同样必需，否则 `Marco` 会被 ③ 误杀。
>
> ⚠️ **改判据之前先把这张表重新量一遍。** 上一轮 8 个假阳性就是"按直觉改规则"改出来的：
> 看起来"句首大写不算证据"已经够严了，实际在一场 53 段的真实播客上松得离谱。
>
> **判定与计数是两件事，别混。** 一个键一旦被判为专名，它的**每一次**出现（含段首那次）
> 都要计入 `hits` —— 否则界面上会显示"已经听到 1 次"而实际听到 2 次，**给用户看的数字就是假的**。
> （`Erica` 在 #47 是 3 次，含段首那次。）我给计数环节加过一次"只数非句首的"过滤，
> 自检立刻报「Marco 出现次数应为 2」。
>
> **回归用例用的是真实转录**（会话 #47 的 53 段 / #43 / jfk.wav），不是编的句子 ——
> 编的句子抓不出这类"在自然语流里才会出现的小写形式"。
> **规则是量出来的，不是想出来的**：先取证（`extract_case_probe.py`），再定判据，再补回归。

**中文专名（R6/R7，2026-09-17 补）**

中文**没有大小写、没有词边界**，上面那套英文规则一条都用不上。不补的话，
中文会议里「张伟负责下周的报价」**一个候选都抽不出来** → 知识库不长 →
没问题可问 → 三条腿没输入 → **"越用越懂你"在中文场景下完全不成立**。
而中文会议正是这个产品的主场（也是评审和用户的真实场景）。

| 规则 | 依据 | 例子 |
|---|---|---|
| **R6 人名** | **姓氏字 + 佐证**。光看"张伟"两个字判断不了是不是人名（"张开""明显"都能凑出来），所以必须有佐证才认： | ① 后面跟**称谓**（最强）：张总 / 李经理 / 王工 / 王老师<br>② 后面跟**动词**：张伟负责 / 李小明提出 |
| **R7 机构/项目名** | **后缀**是强信号（"项目""科技"几乎只出现在专名后面）。从后缀往左扫、**碰到虚词就停**、只取 2~4 字 | 凤凰项目 / 星辰科技 |

两道防假阳性的闸（都是被实测抓出来的，见 §8.8）：
- **人名里不能含虚词**（从第 2 个字起查）：否则「那边**的**反馈」会抽出个假人名「边的」；
  第 1 个字不查，因为「于」本身是常见姓氏
- **弱后缀**（项目/系统/平台）要求本场出现 **≥2 次**才报。
  刻意**不收** 计划/方案/产品/版本/部门/团队/中心 —— 与普通名词重合度太高
  （实测「交付计划」会被当成专名）。报错的代价不是"多一条数据"，
  而是**用户被问一个蠢问题**（§1.3 提问是稀缺资源）

> ⚠️ **中文抽取目前只在手写语料上验过，没有真实 ASR 输出可用。**
> `tools/zh_extract_cases.py` 用的是手写的中文会议片段 —— 它只能验
> "规则在中文会议的语言形状上认不认得出"，**验不了真实 ASR 输出上的准确率**
> （ASR 会有同音错字、有时不加标点）。真实验证：
> 录一场中文会议 → `Translator.exe --extract <会话id>` → 按结果回来调词表。

**明确不做**（写下来免得以后当 bug 修）：不抽多词英文专名（`New York` / `Li Wei`）；
中文只做上面两条结构性规则，**不做分词** —— 那要词典 + 统计模型，
而且噪声会直接变成"问用户蠢问题"。
...
```
Confirmed 知识 ──→ 翻译约束 / 术语统一
              ──→ 摘要背景（大模型知道 EchoMind 是项目名，不是错别字）
              ──→ TermFixer 识别后纠错（确定性，不会幻觉）
              ──→ ⚠️ Whisper initial_prompt —— **2026-09-19 起默认不用知识库**（见下）
```

下一场识别更准 → 抽取更准 → 知识更准。**这个正反馈环就是产品壁垒。**

> ### ⚠️ 2026-09-19：知识库**默认不再**进 `initial_prompt`（有实测证据的产品决定）
>
> 识别腿从 2.5 做到现在，只有"词进去了"的验证，**没有"进去之后有没有用"**。
> 于是补了 A/B（`tools/asr_prompt_ab.py`）：拿一段**内容已知**的音频
> （11 秒肯尼迪演讲，里面没有任何词表里的词），对比给不给提示两档，**每档重复 3 次**。
>
> | 档位 | 转录 |
> |---|---|
> | N=0 无提示 | …can do for you. **ask what you** can do for yourself… |
> | N=5 真实库的 4 个词 | …can do for you. can do for yourself… ← **吞掉了 "ask what you"** |
> | N=4 4 个编造词 | …Thank you **for watching!** ← **幻觉**（音频里没有这句） |
> | N=40 40 个词（原产品上限） | 4 段 → 93 字符，整句 "Thank you" 不见了 |
>
> **每档 3 次结果完全一致**，所以差异是 prompt 造成的，不是运行噪声。
> 结论：**喂进去的词若与音频内容无关，Whisper 会漏字、会幻觉，档位越高越糟。**
> 而知识库是**跨会议全局累积**的 —— "和本次内容无关"恰恰是常态
> （上周学的播客人名，这周开项目会照样被喂进去）。
>
> 另一头，"有用"那一半**至今没有证据**：要证明它需要一段**含易错专名的音频**，
> 目前没有（jfk 那段在无提示时就基本正确，没有改进空间）。
>
> **取舍：不拿"没被证明有用"的机制，去换"已被证明有害"的代价。**
> 收益那一半有**确定性**落点且不受影响：`TermFixer`（识别后按词表纠错，不会幻觉，
> 拿到的是**全量**词表）+ 翻译约束 + 摘要背景。
>
> 想启用：`--asr-prompt-kb N`。留这个开关是因为"有用"那一半**可能真的成立**，
> 等有合适的音频就能验；在有证据之前默认 0。
> `--terms` 会**解释**为什么提示是空的（而不是静默空着）。

现状对照（2026-09-14 实测；2026-09-19 补注）

| 这条腿 | 现状 |
|---|---|
| → Whisper `initial_prompt` | ⚠️ **通路在**（`build_prompt_terms()` → `terms_to_prompt()` → `engine.set_initial_prompt()`），但**默认只吃 `--glossary` 文件**；知识库要显式 `--asr-prompt-kb N`。**收益未经验证，已知有害** |
| → 识别后纠错 | ✅ **已通且是主要落点**：`TermFixer` 修 `Erikka → Erika` 这类同音误识别（确定性，**不会幻觉**） |
...
2026-09-19 识别腿按实测证据改为默认关闭（知识库侧）。**
...
#### ③b 用户教它的**含义**入库（2026-09-17 补，步骤 2.7）

**这一环决定了这个产品是"记词表"还是"懂业务"。**

在 2.7 之前，一条知识能表达的全部内容是"这个词该写成什么样"（`key → value`）。
于是系统能做的只有"把 Erica 认成 Erica"，**永远做不到知道 CO-RE 是什么**。
而用户能教给系统的最有价值的东西恰恰是后者：

```
第一次会议：用户提到 CO-RE。系统只知道"多了个陌生词"。
    ↓ 会议结束主动问（§6.6 的 ask_definition 规则）
「你提到了「CO-RE」。它是你们项目里的重要概念吗？是的话，它指什么？」
    ↓ 用户答一句话
「指 Compile Once – Run Everywhere，是我们 eBPF 项目的核心方案。」
    ↓ 存进 knowledge.definition
第二次会议：摘要背景里带上这句 —— 模型知道 CO-RE 不是错别字、是技术方案；
            search_knowledge 也能检索到它（它**在转录里根本不存在**，
            不索引就等于"用户教会了系统、系统却检索不到"）
```
...
| 决定 | 理由 |
|---|---|
| **含义绝不进识别提示 / 翻译约束** | 一句中文描述塞进 Whisper 的 `initial_prompt` 只会挤掉真正的专名（那串只有 224 token 预算），而 Whisper 不会报错，**只会识别得更差**。出口只有两个：摘要背景 + 检索 |
| **这一问排在"高频未确认"之前** | 它本来就是**一问两用**（需求原话：「它是重要概念吗？如果是，它指什么？」）。排在高频后面的话，hits=3 的术语会先被问"我打算把它记成术语，对吗？"，用户按个 y 就结束 —— 拿到"确认"却**永远拿不到"含义"** |
| **每场最多问 1 个** | 别的规则按个 y 就行，这个要用户**打一整句话**。一场问三个他就不答了（§1.3）。剩下的术语下次接着问 |
| **单独一条写路径 `set_definition()`，不走 upsert** | upsert 的同值分支会 `hits + 1`，而"用户答了一句它指什么"**不是又听到一次**。实测走 upsert 之后 `EnglishPod` 的 hits 从 3 变成 4 —— 上一场明明只听到 3 次，界面上却写 4 次 |
| **长度上限单独放宽到 300 字节** | 默认 60 字节是给专名的，而「Compile Once – Run Everywhere，是我们 eBPF 项目的核心方案」就已经 60+ 字节 —— 用默认值会让**最自然的回答被静默判成跳过**，表现为"用户教了但系统什么都没记住" |
| **只问 `kind == term`** | "Erica 指什么"是个没有意义的问题。机构/项目名勉强能问，但那更像闲聊，而提问是稀缺资源 |
| **答 `y` 只确认、不编造含义** | 用户说"是重要概念"但没说它是什么时，`definition` 保持空。下次会话还会问 —— 编一句出来会污染摘要 |

**表结构**：`knowledge.definition TEXT`（2.7 加的列，走 `ALTER TABLE` 迁移；
`CREATE TABLE IF NOT EXISTS` 永远不补列）。
FTS 索引也加了这一列 —— 而 `CREATE VIRTUAL TABLE IF NOT EXISTS` 同样不补列，
所以老库要把 FTS 表和触发器**整块 DROP 重建**再从内容表 `rebuild`（内容表不动）。
实测老库第一次被打开会打印 `[DB] knowledge_fts 缺少 definition 列，正在重建索引`。

**⚠️ 已知限制**：中文没有分词，FTS 的 unicode61 把一整串汉字当一个 token，
所以检索中文含义只能**前缀匹配**（`MATCH '专门教*'` 命中，
`MATCH '日常'` 不命中）。要真正支持中文检索得换分词方案 —— 属于遗留项。
...
**怎么看见这条红线在生效（2.6c 补）**：跑 `Translator.exe --terms --db <库>`。
它不加载任何模型，几秒钟打印出**真正会送给引擎的那一串**（`initial_prompt` / 逐条翻译约束 / 摘要背景）。
在此之前只能靠真跑一场会、在启动日志里翻 `[识别提示]` 那一行 —— 要录音、要等 Whisper 加载上百秒，
**验一次的成本高到没人会验**，于是库被污染了没人发现，直到用户自己看日志问"这些词哪来的"。

> ⚠️ **"已经被用户确认过"不等于"是对的"。** 2.6c 的污染事故里，8 个假专名全都是用户
> **逐条按了确认**的。人面对"是否记住了 down？"会顺手按 y。**把确认当成质量保证是错的** ——
> 上游该拦住的东西不能推给用户。这也是 §6.7「提问是稀缺资源」的另一个理由：
> 每问一个蠢问题，都在消耗用户对这套记忆的信任。

**`archived` 是清理的唯一手段，不是 `DELETE`。** 脏条目降级成 `archived` 就不进提示/约束了
（`usable_as_constraint()` 只认 `confirmed`），效果等价于删除，但**可逆、可复查**。
`tools/knowledge_audit.py` 里**没有 delete 分支** —— 删除是不可逆的，这个项目已经因为
一次误删丢过用户的 4 条 confirmed（见 §8.8）。历史永远只追加。
...
| 信号 | 判定 | 例子 |
|---|---|---|
| **译法不一致** | 同一个键的值变过多次（历史里有两个以上不同值） | 同一个键先记「艾瑞卡」又记「埃里卡」 |
| **写法冲突**（2026-09-17 补） | 库里两条记录，**键只差一个字母**（同首字母、长度 ≥4、编辑距离 ≤1） | `Erica`/`Erika`、`Marco`/`Marko` |
| **高频未确认** | Candidate 且 `hits >= 3` | 一场会说了 8 次的「Phoenix」，库里没有 Confirmed 记录 |
| **低置信度专名** | Candidate 且识别置信度 < 0.5 | 置信度 0.4 的人名 |
| **本场新见** | Candidate 且 `source_session == 本场` 且是专名 kind | 自动抽取（§6.4①）的出口 |
| **问含义**（2026-09-17 补） | `kind == term` 且 `definition` 为空 | 「你提到了「CO-RE」。它是你们项目里的重要概念吗？是的话，它指什么？」← **闭环的核心那一问**，见 §6.4③b |
| **旧值 ≠ 新值** | 与已有 Confirmed 值冲突 | ASR 从 Whisper large-v3 换成了 Qwen ASR |

"旧值 ≠ 新值"那条就是建议稿强调的 **Event / Decision 一等实体**：只记"当前值"会丢掉"它是怎么变的"，而**"变了"恰恰是最该主动告诉用户的事**。

#### 写法冲突：为什么不问就等于让用户瞎猜（2026-09-17，用户真跑会话 #13）

播客主持人叫 **Erica**。会话 #43 里 Whisper 听成 `Erica`，会话 #13 里听成 `Erika`。
旧代码把两条记录当成**互不相干的知识**，于是问了两个独立的问题：
...
下一场的识别提示变成 `initial_prompt = "Marco, Erika"`（**错的那个词开始生效了**）。

**正确答案其实一直在我们自己手里**：库里就是有两条只差一个字母的记录，
这本身就是"疑似同一个实体"的强信号。这种情况下把判断推给用户之前，
必须先把冲突摆出来 —— 否则用户只能瞎猜，而猜错会被记成约束。

所以现在合并成一问，并且：

| 做法 | 理由 |
|---|---|
| **一次问完，列出全部选项** | `我这儿记了 2 种写法：1) Erica（听到 3 次）　2) Erika（听到 2 次）。是同一个吗？哪个对？现在按「Erika」在用。` |
| **显式说出"现在按哪个在用"** | 用户要知道改动的影响面，而不是靠选项位置去猜 |
| **选项按 hits 降序，不按 confirmed 排前面** | 这个真实案例里 confirmed 的恰恰是**错的**那个（`Erika` 2 次）而对的只有 3 次的 candidate（`Erica`）。按状态排序等于拿错误答案引导用户 |
| **已确认的条目也纳入分组** | 只扫 candidate 的话这个冲突**永远问不出来** —— 而 confirmed 的那个正在进提示词，错的那个**已经在生效了**。这和"值被改掉了要重新问"是同一个道理（§1.3 防的是"同一件事反复问"，不是"新出现了矛盾也不许问"） |
| **其余写法降级为 `archived`** | 它们是同一个词的**听错版本**，不是另一条知识。留着会继续被单独问，甚至被确认后进提示词。降级而非删除，因为"我听过这个写法"本身是有价值的历史 |
| **合并条件卡得很紧** | 同首字母 + 长度 ≥4 + 编辑距离 ≤1。判错的代价是"让用户在两个无关词之间做选择"，比不问更糟。两字母词（`PC`/`PB`）一律不合并 |
| **组里每个成员都要记一次"问过了"** | 只记第一个的话，剩下的下轮还会被单独问 —— 用户会觉得"我不是刚回答过这个吗" |

同一次真跑还带出另一个 bug：用户选中**本来就是 confirmed** 的那个写法时，
`set_status(id, "confirmed")` 会失败（`can_promote` 里 `from == to` 返回 false，
"状态没变就不是一次状态变更"），于是用户答对了却被告知
`[失败] 状态不允许从 confirmed 变为 confirmed`。现在会先读当前状态，
已经是 confirmed 就算成功 —— **别把"无需变更"当错误报给用户**。

#### 提问措辞：用户原话"感觉根本不是人类的会问出的东西"（2026-09-17）

旧措辞有两个毛病，第二个比第一个严重得多：

| 旧措辞 | 毛病 |
|---|---|
| `已经听到 N 次「X」，一直没确认过。它是对的说法吗？` | ① 「说法」说的是**措辞/论断**，拿来指一个专名是词不达意；② "一直没确认过"是**系统视角**，用户不关心 |
| `第一次听到「X」。这个词的写法对吗？` | 把"拼写对不对"推给一个**刚听到、还没看到字**的人 —— 他没法回答 |

**更根本的问题**：这两句都在问用户**无法回答**的事。用户在听音频，他不知道那个词该拼成什么样。
真正属于他的决定是"**要不要让我把这个词记下来当名字用**" —— 这才是他能回答、也愿意回答的问题。

所以改成：

```
1. 我这儿记了 2 种写法：1) Erica（听到 3 次）　2) Erika（听到 2 次）。是同一个吗？哪个对？现在按「Erika」在用。
2. 「EnglishPod」这场听到 3 次了，我打算把它记下来当术语用。对吗？
3. 新听到「Samuel」，要记住它吗？
```

**拼写对不对是系统该自己收敛的事**（靠写法冲突规则把候选摆出来问），
不该变成一个"请用户凭空确认拼写"的问题。

> ⚠️ 上面这段是 2.6d 的中间版本。**它在 2.8 又被推翻了一半** ——
> `「EnglishPod」这场听到 3 次了` 里的"这场听到 3 次"是个**错的数字**
> （`hits` 是跨会话累加的），而且"听到 N 次"仍然是我们的记账方式。
> 2.8 把文案整体搬进了独立的交互层，见下一节。

#### 2.8 交互层：为什么"该问什么"和"怎么说"必须分开

用户原话：**「感觉自己是在给数据库做标注，而不是在教 AI 学习。」**

在这之前，面向用户的中文是**拼在 `KnowledgeGap.cpp` 里**的 —— 和"该问什么"的
判定逻辑写在同一个函数里。后果不是"文案难看"，是**内部状态顺着这条路漏了出去**：

| 漏出去的东西 | 用户看到的 |
|---|---|
| `std::to_string(k.hits)` | 「已经听到 4 次」 |
| `fmt_conf(k.confidence)` | 「识别置信度只有 0.40」 |
| `asked_count`（措辞） | 「跳过（以后还会再问，最多再问一次）」 |
| status 字面 | 「已确认：Marco」 |

而且改一句话要去改检测规则的代码，验一句话要先把知识库和检测规则都跑起来。

**现在的分层（箭头不回头）**：

```
KnowledgeGap   逻辑：该问什么          → GapQuestion（**只有事实**）
...
| 约束 | 怎么保证 |
|---|---|
| **`Interaction.h/.cpp` 不许 include `KnowledgeGap.h` / `KnowledgeStore.h`** | 结构性约束，不是风格偏好 —— 一旦能拿到 `KnowledgeItem`，迟早会有第一个文案顺手把 status / confidence 打出去。**让结构上拿不到**，比写一句"注意不要泄漏内部状态"可靠 |
| `Prompt` **刻意不含** hits / asked_count / confidence / status | 它只描述"要把这件事跟人说清楚，需要知道什么" |
| `GapQuestion` 里**没有** `question` 字段了 | 只要"该问什么"的代码里能拼出文案，泄漏就会重演 |
| 枚举按**"用户需要做什么决定"**分，不按内部规则分 | 内部七条规则 → 用户侧八种问法，多对多。规则增删不影响用户看到的问法种类 |
| 文案可以**单独自检** | 不需要数据库、不需要缺口检测、不需要 `KnowledgeItem` |

**提问按知识类型走不同的说法**（同一个内部规则，用户侧看到不同的问题）：

| 内部规则 | 知识类型 | 用户看到 |
|---|---|---|
| `NewlySeen` / `HighFreqUnconfirmed` | person | 「我注意到你多次提到 Marco。这是正确的人名写法吗？」 |
| 同上 | project | 「…这是个项目名吗？以后我会按这个名字来记。」 |
| 同上 | term | 「…这是你们项目里的一个技术术语吗？」 |
| `AskDefinition` | term | 「…这是你们项目里的一个技术术语吗？如果是，它指什么？」 |
| `ValueChanged` | 任意 | 「我之前记得「凤凰项目」是「10 月上线」，刚才我听到的是「11 月上线」。需要更新我的记录吗？」 |
| `ConflictingSpellings` | 任意 | 「这个名字我这儿有两种写法：「Erica」、「Erika」。是同一个人吗？哪个对？（现在用的是「Erika」）」 |
| `LowConfidenceName` | 任意 | 「刚才有个名字我没太听清（X）。正确的写法是什么？」 |

**每一条都要说清"我为什么觉得它值得记住"** —— 这是让用户感到闭环的关键一句。
注意用的是**用户自己的经历**（"我注意到你多次提到"），不是我们的计数器（"已经听到 4 次"）。

**回执是体验的落点**，不能说"已确认"，要说清"以后我会拿它做什么"：

```
好的，我记住了 Marco。以后再遇到这个人名，我会按这个写法处理。
记住了：「EnglishPod」是指英语播客节目，专门教日常英语表达。以后整理纪要和检索的时候我会用上这个理解。
好，我更新了：「凤凰项目」现在是「11 月上线」。之前记的「10 月上线」我留作历史，以后不会再用它。
先不记这条。                      ← 不提"以后还会再问""最多再问一次"
```

**`Familiarity` 为什么是定性而不是次数**（这条是真 bug，不只是文案）：

`hits` 是**跨会话累加**的（`upsert` 里 `hits = hits + 本次次数`）。所以
「这场会话里听到 4 次」**是假的** —— 实测 `erica` 的 hits=9 是 #43 那场的 3 次
加 #46/#47 若干次累加出来的。定量描述在跨会话语义下站不住，定性描述站得住：

| 档位 | hits | 说法 | 为什么成立 |
|---|---|---|---|
| `FirstTime` | 1 | 「你刚才提到了 X」 | — |
| `AFewTimes` | 2 | 「我注意到你不止一次提到 X」 | 跨会话累计 ≥2 次，且没说"这场" |
| `ManyTimes` | ≥3 | 「我注意到你多次提到 X」 | 同上 |

**自检里有一条禁用词表**（⑱）：`已经听到` / `置信度` / `candidate` / `已确认：` /
`约束` / `知识库` … 任何一种出现在**任何**一条文案里就算失败。
用笨办法的理由：漏一个字段的代价是用户看到莫名其妙的东西，而这类文案改一次很难被发现
（没人会去读自己的输出）。**这条表当场就抓住了我自己写的一句「不会再拿它当约束」。**

#### 2.8f 开工语：让"复用"看得见

**真实数据暴露的问题**：用户跑完三场演示后复盘发现，**"第二次知道"这一拍在屏幕上完全看不见** ——
#1 和 #2 的 `initial_prompt` 一模一样（说明复用确实发生了），但没有任何一句话说出来。
复盘只能靠事后工具翻库。
...
```
[复用] 开工之前先说一句：我带着之前学到的 4 条知识。
          · EnglishPod —— 指的是这个英语博客的名字，是一个练习英语口语和听力的博客
          · CarsPacked —— 装车，将行李或物品装上车
          · TV —— 指的是电视节目的缩写
       （识别时会偏向这些词，翻译时统一写法）
```
...
```
[复用] 我这边还没有关于你的任何记录，这一场从零开始听。
```

—— 否则用户分不清"系统没有记忆功能"和"系统有记忆但现在还是空的"，而**后者正是"第一次不知道"的起点**。

**输出顺序从"人"到"机器"**：先给人看的一句（`[复用]`），再是内部计数（`[知识库]`），
最后才是实际送进引擎的原始串（`[识别提示] initial_prompt = ...`）。
最后那行必须留着 —— 它是"诊断必须显示真实的东西"那条原则的落点。

**副作用是好的**：数据一坏就肉眼可见。实测损坏的库打出来是 `· 是的`（没有含义、没有说明）。

#### 2.8e 「是的」事故：表外的肯定说法会变成一条被写进库的值

用户在三场演示里答了「是的」，而肯定词表里只有「是」「对的」「好的」——
于是它掉进"其余内容一律当用户给出的正确写法"分支：

```
person marco = 是的   status=confirmed
person erica = 是的   status=confirmed
→ initial_prompt = "EnglishPod, CarsPacked, 是的, TV"，而 Marco / Erica 整个消失
```

**用户答了一句"是的"，系统把两个人名删了、塞进去两个字。** 而他看到的是
「好的，改成「是的」了。」—— 听起来还挺合理，这才是这类 bug 最可怕的地方。

**修法不是继续加词**（"是的/是啊/对呀/好啦/行的吧"穷举不完），而是
**剥掉句尾语气助词再查表**（`strip_trailing_particles()`）：核心词 + 助词的**整类**一起解决。
两张触发表同时也补厚了（项目既定规矩：触发表宁宽勿漏，往这里加词是安全的）。

> ⚠️ **已知脆弱点（本轮没修，属遗留）**：**任何不在触发表里的自然肯定说法，
> 仍然会变成一条被写进库的值。** 剥语气词覆盖了最常见的一类，但表本身仍是这类事故的边界。
> 真正的兜底是**写库前回显一次**（"我要把 Marco 改成「X」，确定吗？"），那需要一次额外的往返。
...
| **2.6b** | **中文专名抽取（§6.4① R6/R7）**：中文没大小写没词边界，不补则中文会议学不到任何知识 | L1 | 2.6 | ✅ |
| **2.6c** | **抽取精度回归修复 + 库清理 + `--terms` 诊断**（用户真跑 #47 暴露：8 个假阳性真进了 `initial_prompt`） | L1 + **L3** | 2.6b | ✅ |
...
| **2.8c** | 措辞对齐：`kind=term` 里混着中文项目名，改说「重要概念」 | L1 | 2.8b | ✅ |
| **2.8d** | 模型加载日志静音（真实启动 1483 行 → 68 行）+ `--verbose` 还原 | L1 + **L3** | 2.8c | ✅ |
| **2.8e** | **修「是的」数据损坏**：剥句尾语气词，把"核心词+助词"整类解决 | L1 + **L3** | 2.8d | ✅ |
| **2.8f** | **开工语：让"复用"看得见**（用户三场演示暴露"复用是静默的"） | L1 + **L3** | 2.8e | ✅ |
...
#### 第 5 阶段 · Agent 化（**插在第 4 阶段之前做**）

> 顺序理由：赛题赛道维度 20 分里有「工具调用」，原话是“是区别于问答机器人的关键”；
> GUI 影响的是完成度观感，Agent 影响的是**定性**。详见下面《第 5 阶段 · Agent 化》小节。

| # | 做什么 | 状态 |
|---|---|---|
| 5.1 | Tool Registry（只读工具）+ `--tools` | ✅ |
| 5.2 | 最小规划循环（function calling + 预算 + `--audit`） | |
| 5.3 | `--ask`（吸收原 2.7，做成 agent 的退化情形） | |
| 5.4 | 写工具 + 闸（**知识侧不给写权限**） | |
| 5.5 | Action 落库 + 跨会话聚合（原 2.8，比原估的大） | |
| 5.6 | `--report` 派活入口 | |
| 5.7 | Evidence（原 3.3 提前） | |
| 5.8 | Demo 数据 + 演示脚本 + 兜底 | |
...
#### 第 5 阶段 · Agent 化 —— 从"会议工具"到"自主执行数字员工"（8 步）

> **为什么它插在第 4 阶段前面**：赛题的赛道维度有 20 分，评的是
> **任务理解与自主规划 / 工具调用与环境交互 / 结果交付与可验收性 / 场景价值与端到端闭环**，
> 而其中「工具调用」赛题原话写着"**是区别于'问答机器人'的关键**"。
> GUI（第 4 阶段）影响的是完成度观感，Agent 影响的是**定性**。
> 顺序决定的是"先补 20 分的赛道维度，还是先补好看的壳"。

##### 目标形态

```
现在：打开 → 听 → 结束 → 纪要            （被动：它是一台会议记录仪）
之后：用户一句话派活 → 它自己规划 → 调工具 → 交付可验收成果
                                          （主动：它是一个数字员工）
       而且"打开就听"完整保留 —— 那是它的**感知**能力，别的智能体作品没有
```

##### 两个已定的决策（2026-09-17，用户确认）

| 决策 | 结论 | 理由 |
|---|---|---|
| **① agent 提到 GUI 前面** | 第 5 阶段先做，4.x 推迟 | 20 分 vs 观感 |
| **② 知识库不给 agent 写权限** | agent 只能**提出候选**、**由用户确认**；它永远不能自己写 `confirmed` | 否则等于把 §6.5 红线打开 —— 一个会自主写约束的 agent 会污染整条链路，而且用户看不出来 |

> ② 的体感补偿：把确认做成一键（UI/报告里显眼位置），用户感受接近"自动更新"，
> 但红线一点没动。
...
| # | 做什么 | 关键设计 | 状态 |
|---|---|---|---|
| **5.1** | **Tool Registry**：统一工具接口，把已有能力包成**只读**工具 | 见下"工具清单"；结果必须**限长**（进模型上下文）；错误一律**变成回话**不抛异常 | ✅ 完成 |
| **5.2** | **最小规划循环**：function calling + Agent State + 预算 + 超时/失败/取消/重试 + `--audit` 逐步打印 | ⭐ 把"调 LLM"也**抽成可注入回调** —— 和 2.4 的 `LineReader` 同一招，于是**规划循环能进 L1**（喂脚本化的 tool call 序列，不联网） | 未开始 |
| **5.3** | **`--ask`** ← 吸收原 2.7 | 它**不是独立实现**，而是 agent 的**退化情形**（单次工具调用、不规划）。否则将来有两份代码 | 未开始 |
| **5.4** | **写工具 + 闸** | 知识侧只给"提出候选 + 走用户确认"；行动项侧允许写但带来源标记 | 未开始 |
| **5.5** | **Action 落库 + 跨会话聚合**（= 原 2.8，比原估的大） | ⚠️ `actions` 表**建了但从来没人写入**（0 行）→ 得先落库、再判"跨会话是不是同一件事"、再做状态流转 + 幂等 | 未开始 |
| **5.6** | **`--report` 派活入口**：自然语言 → agent → 交付文档 | **复用 `DeliverableWriter`**，不开新路径 | 未开始 |
| **5.7** | **Evidence**：报告里每个结论可跳回段落 | 把原 3.3 提前 —— 没有它，"可验收"站不住 | 未开始 |
| **5.8** | **Demo 数据 + 演示脚本 + 兜底** | 种子数据保证现场一定演得出来；真实录音证明它真能用 | 未开始 |

> **Demo 场景（已定）**：「**整理我过去一周关于凤凰项目的工作并生成周报**」
> 它用到全部已有能力、交付物可验收、一条命令演完。
> ⚠️ **它需要数据前提**：现在库里只有 EnglishPod 播客和 jfk.wav。
> 所以要**合成几场演示会议**（标清楚是演示数据）**加上几场真实录音**。

##### 工具清单（5.1 已实现 5 个，全部只读）

| 工具 | 底层 | 说明 |
|---|---|---|
| `search_knowledge` | `KnowledgeStore::search()` ✅ | 检索长期记忆。返回**状态**（confirmed/candidate）、出现次数、原话证据 —— agent 必须能区分"用户确认过的"和"模型猜的" |
| `knowledge_history` | `search()` + `history()` ✅ | 某条知识的变化历史（"这个说法什么时候改的"） |
| `list_sessions` | `list_sessions()` ✅ | 最近会话；支持 `days`（过去 N 天）；返回 `has_report` —— agent 靠它决定"读现成的纪要"还是"只能看转录" |
| `get_session` | `fetch_segments()` ✅ | 某场转录（限长，默认 30 段 / 最多 60） |
| `get_session_report` | 交付物 HTML/MD ✅ | **读已经生成过的纪要** —— 又快又准，不用重新总结 |
| `ask_user` | `run_confirmation` 的 `LineReader` 接缝 ✅ | 5.4 接上 |
| `list_actions` / `create_action` / `update_action` | ❌ **不存在，且表里没数据** | 5.5 |

**刻意不注册的**：任何写操作；`list_actions` 系列（现在注册只会让 agent 看起来像坏了）；
"重新总结一场会话"（贵、慢，90% 情况已有交付物可读）。

##### 三个必须做对的地方（都是"错了会静默失败"的）

1. **工具结果必须限长**。它进模型上下文，不设上限就是烧 token + 撑爆上下文，
   而多步循环会把这个问题按步数放大。每个工具自己封顶（不是靠循环层统一裁）。
2. **错误必须变成"回一句话"，不能抛异常**。模型会猜错工具名、会吐半截 JSON ——
   这两件事在循环里**一定会发生**。异常 = 整个任务崩掉；回话 = 模型有机会改口。
   （未知工具时把"能用的工具列表"回给它，实测它看到列表就会改口。）
3. **回归必须覆盖"模型实际拿到的那串 JSON"**。schema 拼歪、中文转义丢了，
   在 C++ 里看不出任何异常 —— 只有解析时才炸，而那时问题已经到模型那边了。
   所以 `json_ok()` 是一条断言，`tools/agent_tools_check.py` 也要跑（跨进程边界）。
...
|---|---|
| 自检 | **31 组** |
| 新增工具 | `tools/agent_tools_check.py`（工具层回归）· `Translator.exe --tools`（看 schema / 手动跑一个工具） |

> ⚠️ **`--tools` 存在的理由**：Agent 出问题时，"模型不会用工具"和"工具本身坏了"
> 在日志里长得一模一样（都表现为"最后没给出有用答案"）。
> 打印 schema 能看出描述写歪没有；跑一次能看出工具在真实数据上返回什么。
> 而**命令行和 Agent 走同一个 `ToolRegistry::call`** —— 不给自己留第二条路径。
...
| ⑳ **`strcmp` 用在"前缀判断"上** | 写中文抽取时用 `strcmp(rest, "负责")` 判断"后面跟的是不是动词"，而 `rest` 是**后面剩下的整串**（"负责下周的报价"）—— 只有动词正好落在串尾才匹配得上。后果：「张伟负责下周的报价」里的**张伟完全抽不到**，而「那边的**反馈**」里那个假人名「边的」**反而被抽到**（因为"反馈"正好在串尾） | 判断"是不是以某个词**开头**"用 `strncmp`；判断"是不是**恰好等于**"才用 `strcmp`。这个 bug 是被批量用例抓出来的 —— 单个用例很可能刚好避开 |
