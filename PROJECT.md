...
**三条腿到 2026-09-14 全部打通。** A/B 实测（同一句、同一模型、贪心采样所以可复现）：

| | 输入 | 输出 |
|---|---|---|
| 无术语表 | `How are you, Erica? Marco, I'm doing really well.` | **埃里卡**，你怎么样？**马可**，我过得很好。 ← 复现会话 #11 的原状 |
| 有术语表 | 同一句 | **Erica**，你怎么样？**Marco**，我过得很好。 ← 修好 |
...
| 2.5 | **三腿复用接知识库**：Whisper `initial_prompt` / 翻译约束 / 摘要背景，全部改从 `knowledge` 取，去掉对 `terms.sample.txt` 的依赖 | L1 + **L3** | 2.4 + 1.1 | ← **下一个** |
