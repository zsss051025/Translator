...
// ---------------------------------------------------------------------------
// 写工具（步骤 5.4）—— 只有一条路，而且**必须**是"提议"
// ---------------------------------------------------------------------------
//
// 赛题要的"自主智能体"如果只能读不能写，那它在任务中发现"这个名字库里没有"
// 时只能干看着。所以要有写工具。
//
// 但 §6.5 红线是：**模型猜的东西永远不能自动变成约束。**
// 于是这里的关键不是"要不要给写权限"，而是**给哪一种写权限**：
//
//     允许：`propose_knowledge` —— 写一条 **candidate**，等用户在会话结束时确认
//     禁止：任何能把状态改成 confirmed 的路径
//
// 【闸门怎么保证，分三层】
//   ① **代码层**：本函数只注册"提议"工具，实现里只走 `save_candidates()` ——
//      那个函数**写死** `status = candidate`（`ExtractedCandidate` 结构里
//      根本没有 status 字段，所以它连表达"confirmed"的能力都没有）。
//   ② **结构层**：提议出来的东西进的是**正常的候选管道**，
//      而那条管道通往 confirmed 的唯一出口是「用户在会话结束确认时点头」。
//      agent 不在这条路上。
//   ③ **行为级自检**：自检里真调一次写工具，断言落库是 candidate、
//      且 `usable_as_constraint()` 拒绝它 —— 和分诊层"调用前后库条目数不变"
//      是同一个思路：**红线要有可执行证据，不能只是注释里的承诺**。
//
// 【为什么单独一个注册函数而不是并进 readonly】调用方一眼能看出"这一步给了写权限"。
// 正常会话路径（`--report` 之外）**不注册**它们。
//
// ⚠️ 已知取舍：agent 提议的候选会进入缺口检测，下一场会话结束时可能被问一句。
// 这是有意的 —— 提议的价值就在于被问到。而且候选在提问前还要过两道筛
// （通用词表 + 模型分诊），垃圾大部分到不了用户面前。
