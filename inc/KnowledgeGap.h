...
struct GapQuestion {
    ...
    // **它在转录里出现的那句原话**（来自 `KnowledgeItem::source_text`）。
    //
    // 【为什么必须带给分诊层 —— 用户举的例子正好是反例】
    // 我原来给模型判断传的是**词本身**，还在注释里写"人名/项目名的判断不依赖
    // 长上下文"。**那句话是错的**：
    //   用户的花名 `Penny`，全小写 `penny` 是英文单词"便士" →
    //   模型只看到"penny"这个孤立词，很可能判成通用词 → **静默挡掉、根本不会问**。
    // 而"Penny 是不是人名"只有看那句话才知道（"我们的 PM Penny 说…"）。
    //
    // 所以上下文是**判断的必要输入**，不是可选优化。
    std::string evidence;
...
// 那不是"稀缺"，那是**形态不合理**。
//
// 【2026-09-19：4 → 8，因为"该问谁"从一种变成四种】
// 放开了 person / project / product 之后，一场会很容易同时出现
// 2 个人名 + 1 个项目 + 1 个产品 + 2 个术语 = 6 个 —— 4 会**静默截断**后面的。
// 这种截断最难发现（前面几个都问了，用户以为问完了）。
//
// 而 8 这个数不是"稀缺性"，是**形态**：连续打八句话仍然偏多，
// 但先让它不截断，真实的量由「跳过就沉底」去收敛。
constexpr size_t kMaxDefinitionAsksPerSession = 8;
